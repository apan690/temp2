...
## MODULE 7: ADVANCED OPERATIONS *(NEW)*

### Version Control — Check In / Check Out ⭐
```
CHECKOUT (before editing):
- Reserves object exclusively for you
- Creates lock — others see it but cannot edit
- Right-click object → Check Out
- Or: Designer auto-prompts when you start editing

CHECK IN (after editing):
- Saves new version to repository
- Releases lock — others can now edit
- Enter comment describing changes
- Options: Keep Checked Out | Check In | Undo Checkout

UNDO CHECKOUT:
- Discards ALL changes since checkout
- Releases lock
- Object reverts to previous state
- Like git reset --hard

Check In ≠ Undo Checkout:
Check In   → saves changes, releases lock
Undo       → discards changes, releases lock

Object Status Icons:
🔒 Lock   = Checked out by someone else
👤 You    = Checked out by you
📄 None   = Available for checkout
```

### Version History & Compare ⭐
```
VIEW HISTORY:
Right-click object → View History
Shows: Version | Modified By | Modified Date | Comment

COMPARE TWO VERSIONS:
1. View History → select two versions (Ctrl+Click)
2. Click Compare
3. Green = Added | Red = Removed | Yellow = Modified

COMPARE TWO OBJECTS:
Ctrl+Click both objects → Right-click → Compare
Use case: Confirm Dev vs Prod mapping differences

RECOVER OLD VERSION:
View History → Select version → Recover
Restores that version as current
```

### Export / Import (Migration) ⭐⭐
```
EXPORT (Dev → Test → Prod):
Repository Manager → Select objects → Export Objects

CRITICAL OPTIONS:
✓ Export referenced objects — ALWAYS CHECK THIS
  Includes: sources, targets, lookups, shared transformations
  Without this: Import fails with "missing dependency" errors

Export format: .xml file (human-readable)

IMPORT (in target repository):
Repository Manager → Import Objects → browse .xml file

Import options:
- Replace existing objects → Overwrites if exists
- Rename conflicting objects → Keeps old, renames new
- Import referenced objects → Brings dependencies too

COMMON IMPORT ERRORS:
"Connection not found" → Create connection in target repo first
"Folder doesn't exist" → Create folder before import
"Object already exists" → Choose Replace or Rename option
"Missing referenced object" → Re-export with dependencies option
```

### Flagging Strategy ⭐⭐
```
FLAG COLUMN VALUES:
N = New record     → DD_INSERT
U = Updated record → DD_UPDATE
D = Deleted record → DD_DELETE or soft delete (DD_UPDATE + IS_ACTIVE='N')
' ' or NULL = No change → Skip (filter out)

COMPLETE FLOW:
Source (with FLAG column)
    ↓
Filter: FLAG IN ('N', 'U', 'D')   ← FILTER FIRST! Remove unchanged rows
    ↓
Router: RTR_FLAG_ROUTE
    ├─ FLAG = 'N' → Update Strategy (DD_INSERT)        → Target
    ├─ FLAG = 'U' → Update Strategy (DD_UPDATE)        → Target
    ├─ FLAG = 'D' → Update Strategy (DD_UPDATE/DELETE) → Target
    └─ Default    → Error_Log
    ↓
Post-SQL on source:
  UPDATE SOURCE SET FLAG = ' '
  WHERE FLAG IN ('N','U','D')      ← RESET FLAGS after successful load

WHY FILTER FIRST:
1M rows, 999K unchanged → Filter reduces to 1K rows
Router + Lookup only processes 1K (not 1M) = huge performance gain

SOFT vs HARD DELETE for FLAG='D':
Soft (recommended): DD_UPDATE + set IS_ACTIVE='N', keeps history
Hard: DD_DELETE, physically removes row

Flagging vs Timestamp:
Flagging = explicit D flag handles deletes natively
Timestamp = cannot detect deletes (no timestamp on deleted row)
```

### Stored Procedure — Advanced ⭐
```
CONNECTED SP (every row in pipeline):
- IN ports: values passed TO procedure
- OUT ports: values returned FROM procedure
- IN/OUT: passed in, modified, returned
- Use: Row-level enrichment, complex database logic per row

UNCONNECTED SP (called conditionally):
- Called from Expression: :SP.SP_NAME(input_param)
- Returns single value only
- Use: Conditional logic (only call SP for specific rows)

PRE / POST SESSION SP:
- Configured in Session Properties → Components Tab
- Pre-session SP: Disable indexes, truncate staging, archive
- Post-session SP: Rebuild indexes, update stats, send notification

ERROR HANDLING:
On Pre-Session SP Error  → STOP (setup failed = don't proceed)
On Post-Session SP Error → CONTINUE (load succeeded, cleanup failure ≠ fail)
On Row-Level SP Error    → Stop (critical) or Continue (optional enrichment)

DECISION:
Every row needs SP → Connected
Only some rows → Unconnected (IIF conditional call)
Pre/post session setup → Unconnected (session-level config)
Multiple return values → Connected
Single return value → Unconnected
```

### Mapplet Parameters ⭐
```
PURPOSE: Configure mapplet behavior per mapping without creating
         separate mapplets for each variation.

SCOPE: Mapplet parameters are SEPARATE from mapping parameters.
       Mapping $$Params do NOT automatically pass into mapplets.

DEFINE IN MAPPLET:
Mapplet Designer → Parameters and Variables → Add
Name: $$ThresholdValue | Type: Integer | Default: 50000

USE IN MAPPLET EXPRESSION:
IIF(SALARY > $$ThresholdValue, 'Senior', 'Junior')

SET IN PARAMETER FILE:
[FolderName.WorkflowName.SessionName.MappletName]
$$ThresholdValue=80000

Note: MappletName MUST be in section header!
Wrong: [Folder.Workflow.Session]     ← mapplet param NOT picked up
Right: [Folder.Workflow.Session.mpl_SalaryBand] ← correct

PORTS vs PARAMETERS:
Port (I/O)  → Row-level data (different value each row)
Parameter   → Session-level config (same value for all rows)

Row-level data → Input Port
Format strings, thresholds, config → Mapplet Parameter
```
...
❌ 7. Forgetting "Export referenced objects" during migration
❌ 8. Not resetting flags post-load (flagging strategy)
❌ 9. Using mapping parameter inside mapplet (wrong namespace)
❌ 10. Missing mapplet name in parameter file section header
```
...
For Flagging Strategy:
☐ Filter: FLAG IN ('N','U','D') added BEFORE Router
☐ Router has N / U / D / Default groups
☐ Update Strategy per group correctly set
☐ Session: Data Driven mode
☐ Post-SQL resets FLAG to blank
☐ Key columns defined for UPDATE/DELETE
```
...
11. Version Control (Check Out → edit → Check In order)
12. Flagging strategy (Filter first, reset flags post-load)
13. Export referenced objects (critical for migration)
```
...
Unconnected SP Call (from Expression):
:SP.StoredProcedureName(input_parameter)

Mapplet Parameter File Section:
[Folder.Workflow.Session.MappletName]
$$ParamName=value
```
